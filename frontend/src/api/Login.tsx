...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // No response = network error = current API target is unreachable
      // Build a list of nodes to try: baked-in env + cached URLs
      // De-duplicate and filter out the already-failed primary endpoint
      const fallbacks = getFallbackNodeUrls()
      // Also check what base URL the api client currently points at
      const currentBase = (api.defaults.baseURL ?? '').replace('/api/v1', '').replace('/api/v1/', '')
      const allCandidates = [...new Set([currentBase, ...fallbacks])].filter(Boolean)

      for (const nodeUrl of allCandidates) {
        try {
